    ...
    switch ($true) {
        ...
        $Build {
            ...
            if (Test-Path "$root\Output") {
                Remove-Item "$root\Output\RTPSUG\*.psm1" -Recurse -Force
            }
        
            if (Test-Path "$root\src\nuget\tools\RTPSUG.zip") {
                Remove-Item "$root\src\nuget\tools\RTPSUG.zip" -Force
            }
            
            Get-ChildItem $root\src\public\*.ps1 | 
            Foreach-Object { 
                Get-Content $_.FullName | Add-Content "$root\Output\RTPSUG\RTPSUG.psm1"
            }

            $manifest = Import-PowerShellDataFile "$root\Output\RTPSUG\RTPSUG.psd1"
            ...
            Update-ModuleManifest -Path "$root\Output\RTPSUG\RTPSUG.psd1" -ModuleVersion $NewVersion
            ...
            Compress-Archive -Path "$root\Output\RTPSUG\RTPSUG.psd1", "$root\Output\RTPSUG\RTPSUG.psm1" -DestinationPath "$root\src\nuget\tools\RTPSUG.zip"
        }
        ...
        $TestPrePublish {
            ...
            #Import-Module "$root\Output\RTPSUG\RTPSUG.psd1" -Force
        ...
        $DeployToGallery {
            ...
            Publish-Module -Path "$root\Output\RTPSUG" -NuGetApiKey $env:NugetApiKey
